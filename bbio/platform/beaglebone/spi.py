...
class SPI_Bus(object):
  ...
  def begin(self):
    ...
    Connects the BBB to the SPI Bus specified. Initialises both the devices on the bus.
    '''
  ...
  def read(self,cs,n_bytes):
    ...
    Returns the data read from the device as a list whose each element contains 1 byte 
    '''
  ...
  def transfer(self,cs,data):
    ...
    Returns the data read from the device as a list whose each element contains 1 byte 
    '''
    assert self._running == True, "Must call SPIx.begin() before using transfer()"
    assert 0 <= cs < 2, "Chip Select must be 0 or 1"
    if type(data) == str:
      data = [ord(c) for c in data]
    elif type(data) == int:
  ...
  def getBitOrder(self,cs):
    ...
    Gets the Bit Order of the SPI device specified by chip_select to LSB First or MSB First.
    for LSB first : 1
    for MSB first : 0  
    Returns the Bit Order of SPI device.   
    '''
    assert self._running == True, "Must call SPIx.begin() before using getBitOrder()"
    assert 0 <= cs < 2, "Chip Select must be 0 or 1"
    if cs == 0:
  ...
  def setMaxFreq(self,cs,freq):
    ...
    Sets the maximum frequency of the SPI device specified by chip_select to freq in HZ.
    frequency has to be specified in Hz.
    '''
    assert self._running == True, "Must call SPIx.begin() before using setMaxFreq()"
    assert 0 <= cs < 2, "Chip Select must be 0 or 1"
    if cs == 0:
  ...
  def getMaxFreq(self,cs):
    ...
    '''
    assert self._running == True, "Must call SPIx.begin() before using getMaxFreq()"
    assert 0 <= cs < 2, "Chip Select must be 0 or 1"
    if cs == 0:
